...
bool PimocoMount::checkMotionAgainstMountLimits(double haArcsecPerSec, double decArcsecPerSec) {
	// current pos
   	struct ln_equ_posn equ_t0={EqN[0].value, EqN[1].value};
	...
	struct ln_equ_posn equ_t1={equ_t0.ra-haArcsecPerSec/(60.0*60.0), equ_t0.dec+decArcsecPerSec/(60.0*60.0)}; // decSpeed=-haSpeed
   	...
   	// will motion get us at least 0.5 arcsec closer to a compliant state?
   	bool right_dir_t1=((hrz_t0.alt < AltLimitsN[0].value) && (hrz_t1.alt > hrz_t0.alt + 0.5/(60.0*60.0))) || 
        	          ((hrz_t0.alt > AltLimitsN[1].value) && (hrz_t1.alt < hrz_t0.alt - 0.5/(60.0*60.0)))    ;
...
bool PimocoMount::checkMotionAgainstMountLimitsAndStopIfReached(double haArcsecPerSec, double decArcsecPerSec) {
	if(!checkMotionAgainstMountLimits(manualSlewArcsecPerSecRA, manualSlewArcsecPerSecDec)) {
